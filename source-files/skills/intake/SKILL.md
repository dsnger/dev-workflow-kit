...
that today varies by whoever writes the prompt.
...
guardrail the solution has to honor is not designing it — so "auth via
`requireAuth()`" belongs in Section 4, while "add a `requireAuth()` call to the
new mutation" is a HOW leak anywhere else.
...
5. **Tag invariants by grepping AGENTS.md** (not from memory — memory drifts;
   ...
   - extract the idea's domain terms **in English** (translate German first —
     `Einladung`→invitation, `Vorstand`→board — and add synonyms and the verb's
   ...
   - also skim the `## Section`(s) the change plausibly touches (e.g. anything
     permission-related → `## Roles & ACL`), since a concept can live under a
